package {
	...
	public class ChinaMap extends Sprite {
		...
		public function ChinaMap(){
			...
			//var xmlAdress:String = "http://localhost:6657/AjaxHandler.ashx?flag=flashXML";
			//var time:Date=new Date();
			if(xmlAdress!="data/d.xml")
			{
			xmlAdress+="&time="+new Date().time;
			}
			...
			//var urlRequest:URLRequest= new URLRequest(xmlAdress);
			//urlRequest.
		...
		private function registAction(c:DisplayObjectContainer):void {
			...
			for(var i:uint = 0; i<c.numChildren; i++) {
				...
				if(me is MovieClip && me.name != "bg") {
					me.alpha = 0.2;
					(me as MovieClip).value = "无活动监控站点";
					f:for each(var node:XML in mapXML){
						if(node.@id == me.name) {
							//me.alpha = 1;
							(me as MovieClip).alpha = node.@alpha;
